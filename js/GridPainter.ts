    ...
    export class GridPainter {
        ...
        strokeGrid() : void {
            ...
            for (let row = 0; row < this.grid.dimensions.rows + 1; ++row) {
                let y = this.grid.getHorizontalLineY(row);
            ...
            for (let column = 0; column < this.grid.dimensions.columns + 1; ++column) {
                let x = this.grid.getVerticalLineX(column);
                
                this.strokeLine(x, this.grid.top, x, this.grid.bottom);
            }
        }
        
        fillCellsFromModel(): void {
        ...
        fillCell(column: number, row: number) : void {
            let rect = this.grid.getCellRectangle(column, row);
        ...
        rasterizeLine(
            ...
            column1: number, 
            row1: number,
            beforeSetCellValueCallback?: (x: number, y: number) => any): void {
            ...
            let bresenham = new BresenhamRasterizer(
                (x: number, y: number) => {
